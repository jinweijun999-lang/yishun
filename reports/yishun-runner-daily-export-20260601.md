...
2026-06-02 watchdog permission follow-up: GitHub-hosted `GITHUB_TOKEN` cannot read the self-hosted runner inventory API because that endpoint requires repository administration permissions. The watchdog now infers runner outages from watched workflows that remain queued longer than 10 minutes, which is compatible with `actions: read` and still surfaces stale deploy/daily-ops queues.
